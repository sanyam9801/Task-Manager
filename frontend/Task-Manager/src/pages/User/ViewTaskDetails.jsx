...
const ViewTaskDetails = () => {
  ...
  const getStatusTagColor = (status) => {
    switch (status) {
      case "InProgress":
        return "text-cyan-500 bg-cyan-50 border border-cyan-500/50";
      case "Completed":
        return "text-lime-500 bg-lime-50 border border-lime-500/50";
      default:
        return "text-violet-500 bg-violet-50 border border-violet-500/50";
    }
  };
    ...
    if(todoChecklist && todoChecklist[index]){
      ...
      try{
        ...
        if(response.status===200){
          ...
          //Optionally revert the toodle if API call fails
          todoChecklist[index].completed = !todoChecklist[index].completed;
        }
      }catch(error){
        todoChecklist[index].completed = !todoChecklist[index].completed;
